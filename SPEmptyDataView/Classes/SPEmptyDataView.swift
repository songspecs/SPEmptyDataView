...
open class SPEmptyDataView: UIView {
    ...
    func setupSubviews() {
        ...
        self.reload_Btn.addTarget(self, action: #selector(SPEmptyDataView.didTapButton), for: .touchUpInside)
        
    }
    
    @objc func didTapButton(_ button: UIButton) {
        let selector = #selector(UITableView.sp_tv_didTapDataButton)
        ///表示父类是uitableivew
        if superview?.responds(to: selector) != nil {
            superview?.perform(selector, with: button, afterDelay: 0)
        }else{
            let selector = #selector(UICollectionView.sp_cv_didTapDataButton)
